...
def assign_edge_roles(run_config: dict, total_edges: int, global_index: int, master_seed: int, logger) -> str:
    ...
    logger.debug(f"[CONFIG USAGE] assign_edge_roles | pgd_ratio: {pgd_ratio}, fgsm_ratio: {fgsm_ratio}")
    ...
    role_list += ["label_flip"] * num_label_flip + ["gradient_manip"] * num_grad_manip + ["shap_aware"] * num_shap_aware + ["benign"] * num_benign
